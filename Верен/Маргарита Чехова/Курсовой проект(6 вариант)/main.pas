...
var  
  firstSource: source; 
  secondSource: source;
  mainQueue: queue := new queue();  //bufer
...
begin
  ...
  begin 
    ...
    writeln(f,'#1 ',firstSource.getFailureCount,' ',firstSource.getProcessedClaimCount(),' ',firstSource.getCurrentClaimNumber(),' ',
                   firstSource.getFailureCount()/firstSource.getCurrentClaimNumber(), ' ',mainRepository.getterCount(i));
    writeln(f,'#2 ',secondSource.getFailureCount,' ',secondSource.getProcessedClaimCount(),' ',secondSource.getCurrentClaimNumber(),' ',
                   secondSource.getFailureCount()/secondSource.getCurrentClaimNumber());
    mainQueue.nulling();
    if (flag) then
    begin
      //clearWindow();
