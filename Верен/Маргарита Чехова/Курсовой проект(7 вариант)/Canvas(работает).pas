    ...
    begin
      ...
      1 :
        begin 
          ...
          choice := new  TextABC(10,110,10,'Зависимость средненего времени обработки',clRed);
...
begin
  ...
  moveTo(60,490-round(480*mainData.getClaim(j,1,1)/max));
  fillCircle(60,490-round(480*mainData.getClaim(j,1,1)/max),5);
  text := new TextABC(60,490-round(480*mainData.getClaim(j,1,1)/max),12,'0.'+intTostr(round(mainData.getClaim(1,i,1)/mainData.getClaim(1,i,3)*1000)),clBlack);
  Sleep(20);
  for i:=2 to 11 do
  begin
    setBrushColor(clDarkOrchid);
    ...
    text := new TextABC(60+(i-1)*round(640/11),490-round(480*mainData.getClaim(j,i,1)/max),12,'0.'+intTostr(round(mainData.getClaim(1,i,1)/mainData.getClaim(1,i,3)*1000)),clBlack);
