...
function getMin() : integer;//номер источника с миимальным временем заявки
var
  i : integer;
  minTime : integer := 1;
begin
...
procedure setClaims(var r : repository;var m : matrix;j : integer);//заносит данные в хранилище
var 
  i : integer;
begin
...
begin
  ...
  i := 0;
  InitGraph();
  DataInputing(Kmin,flag);//интерфейс
  drawAxesSystem(); //рисует сист. координат
  while (lambda < 3.005) do
  begin
    randomize;
    masSource[1] := new source(1,2.0);
    masSource[2] := new source(2,3.0);
    masSource[3] := new source(3,3.0);
    min := getMin();
    mainPerfomer := new perfomer(lambda);//новый источник
    masSource[min].incrementationProcessedClaim();//увеличиваем заявки на 1
    mainPerfomer.pushClaim(getClaim(masSource[min]));
    mainPerfomer.GenerateTimeRelease(masSource[min].getNextClaimTime());//генерируем время освобождения обработчика
    masSource[min].generateNextClaim();//время до след заявки
    i := i+1;
    changeColor(i);
    Time := 0;
    while (true) do
    begin
      if (mainQueue.isEmpty()) then //берем источник у которого раньше всего заявка кладем заявку в буфер и генерируем след заявку 
      begin
        min := getMin();
      ...
      while ((masSource[1].getNextClaimTime < mainPerfomer.getTimeRelease()) or (masSource[2].getNextClaimTime < mainPerfomer.getTimeRelease())
              or (masSource[3].getNextClaimTime < mainPerfomer.getTimeRelease())) do //пока занят обработчик кладем в буфер, если есть место
      begin
        min := getMin();
      ...
      if (mainPerfomer.getTimeRelease <= mainQueue.get().getNextClaimTime()) then //Если успеет заявка попасть в обработчик то кладется в обработчик
      begin
    ...
    for j :=1 to sourceCount do //передача в хранилище
    ...
    writeln(f,'#1 ',masSource[1].getFailureCount,' ',masSource[1].getProcessedClaimCount(),' ',masSource[1].getCurrentClaimNumber(),' ',
                   masSource[1].getFailureCount()/masSource[1].getCurrentClaimNumber(), ' ',mainQueue.getMiddleCount(1));
    writeln(f,'#2 ',masSource[2].getFailureCount,' ',masSource[2].getProcessedClaimCount(),' ',masSource[2].getCurrentClaimNumber(),' ',
                   masSource[2].getFailureCount()/masSource[2].getCurrentClaimNumber(),' ',mainQueue.getMiddleCount(2));
    writeln(f,'#3 ',masSource[3].getFailureCount,' ',masSource[3].getProcessedClaimCount(),' ',masSource[3].getCurrentClaimNumber(),' ',
                   masSource[3].getFailureCount()/masSource[3].getCurrentClaimNumber(),' ',mainQueue.getMiddleCount(3));
    mainQueue.nulling();//обнуление
  end;
  
  close(inp);
  close(f);
  
   main(mainRepository);// запускает интерфейс
