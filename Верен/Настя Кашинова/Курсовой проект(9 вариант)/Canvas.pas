      ...
      begin
        ...
        begin
          case reading of
            ...
            4 : begin
                  lambda[3] := k[1] + k[2]/10.0 + k[3]/100.0 + k[4]/1000.0;
                end;
            5 : begin
...
procedure MiddleCount(j : integer);
var
  max,temp : real;
  i :integer;
begin
...
begin
  ...
  begin
    ...
    t := new TextABC(200,200,15,s,clOrange);
    setBrushColor(clBlack);
    fillCircle(round(225 + 15*cos(2*i*pi/180)),round(245 + 15*sin(2*i*pi/180)),1);
    Sleep(10);
  end;
end;
...
begin
  ...
  begin
    j := len - i;
    s := new TextABC(round(350 + 250*cos(0.5 + pi + 10.5*i*pi/180)),round(400 + 70*sin(pi + 10.5*i*pi/180)),25,wel[i],clBlue);
    Sleep(200);
  end;
  Sleep(500);
end;
...
begin
  ...
  begin
    j := len - i;
    s := new TextABC(round(350 + 200*cos(pi + 10.5*i*pi/180)),round(300 + 70*sin(pi + 10.5*i*pi/180)),25,wel[i],clRed);
    Sleep(200);
  end;
  Sleep(500);
end;
...
begin
  ...
  begin
    j := len - i;
    s := new TextABC(round(350 + 150*cos(pi + 22.5*i*pi/180)),round(300 + 150*sin(pi + 22.5*i*pi/180)),25,wel[i],clRed);
    Sleep(200);
  end;
  Sleep(500);
end;
...
begin
  ...
  begin
      if (i > 3) then
        line(108*i,30,108*i,512)
      else
        line(108*i,0,108*i,512);
  end;
  line(0,30,840,30);
  for i := 1 to 11 do
  begin
  ...
  text := new TextABC(18,10,12,'Ист1',clBlack);
  text := new TextABC(136,10,12,'Ист2',clBlack);
  text := new TextABC(254,10,12,'Ист3',clBlack);
  
  text := new TextABC(450,5,14,'Буфер',clBlack);
  text := new TextABC(354,40,12,'Ист1',clBlack);
  text := new TextABC(454,40,12,'Ист2',clBlack);
  text := new TextABC(554,40,12,'Ист3',clBlack);
  ...
  begin
    ...
    text := new TextABC(143,35+40*i,12,'0.' + intTostr(round(mainData.getClaim(2,i,1)/mainData.getClaim(2,i,3)*1000)),clBlack);
    text := new TextABC(261,35+40*i,12,'0.' + intTostr(round(mainData.getClaim(3,i,1)/mainData.getClaim(3,i,3)*1000)),clBlack);
    ...
    begin
      ...
      begin
        if (frac < 10) then
          text := new TextABC(213 + p*120,35+40*i,12,intToStr(int) + '.00' + intToStr(frac),clBlack)   
        else
          text := new TextABC(213 + p*120,35+40*i,12,intToStr(int) + '.0' + intToStr(frac),clBlack)  
      end
      else
        text := new TextABC(213 + p*120,35+40*i,12,intToStr(int) + '.' + intToStr(frac),clBlack);
    end;
  end;
  drawArrow(645,5);
end;
